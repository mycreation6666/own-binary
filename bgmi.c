...
void send_udp_packet(int sockfd, struct sockaddr_in* server_addr, const char* message) {
    sendto(sockfd, message, strlen(message), 0, (struct sockaddr*)server_addr, sizeof(*server_addr));
}

// Function to handle sending UDP packets continuously
void send_udp_packets(const char* ip, int port, int duration) {
    int sockfd;
    ...
    while (time(NULL) < end_time) {
        ...
        send_udp_packet(sockfd, &server_addr, buffer);
